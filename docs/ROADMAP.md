...
| **Storage and query pass** | Make the RxDB/Dexie layer as lean as it can be before release. **Key compression** ✅ ([rxdb.info/key-compression](https://rxdb.info/key-compression.html)) — schemas opt in and the storage is wrapped in `db/storage`; measured at roughly 18% smaller rows for entries and 9% for blocks, which gain less because most of a block sits in `contentJson`, whose nested keys are outside the schema. Off in development (`db/key-compression`) so stored rows stay readable in DevTools. Cost measured on memory storage, which isolates the mapping from disk: ~6µs per document written, ~0.25µs per document read. **Indexes** ✅ — `userId` dropped from all three collections (the field stays), `hostnameUrl` added on entries, and `maxLength` added to the indexed timestamps, without which the compound indexes on `updatedAt` could not be used at all. Still to audit: bulk operations everywhere ([#152](https://github.com/lexicora/lexicora-extension/issues/152), done for cascade deletes), JSON conversion RxDB does not need ([#176](https://github.com/lexicora/lexicora-extension/issues/176)), and rewriting unchanged documents ([#146](https://github.com/lexicora/lexicora-extension/issues/146)) — each rewrite bumps the revision and would later be replicated. |
...
| **Release prep** | Version bump, README scope statement, privacy policy, store listing copy and permission justifications. Store submission is deferred until the product is judged ready. Also: Settings → General links to four routes that are not registered (Accessibility, Language, Privacy policy, Miscellaneous) and open the not-found page. The privacy policy one matters for store review; the rest need building, gating or removing. |
