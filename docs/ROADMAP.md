...
| **Storage and query pass** | Make the RxDB/Dexie layer as lean as it can be before release. **Key compression** ([rxdb.info/key-compression](https://rxdb.info/key-compression.html)) shortens stored property names, trading a little CPU for less disk and smaller reads — it changes how documents are stored, so it needs a schema version bump and migration, which is far cheaper now than after v1.0 ships with users' data. Alongside it, audit the operation sites: bulk operations everywhere ([#152](https://github.com/lexicora/lexicora-extension/issues/152), already done for cascade deletes), drop JSON conversion that RxDB does not need ([#176](https://github.com/lexicora/lexicora-extension/issues/176)), and avoid rewriting unchanged documents ([#146](https://github.com/lexicora/lexicora-extension/issues/146)) — each rewrite bumps the revision and would later be replicated. |
