...
| **Firefox pass** | Checked in a real Firefox and working, apart from what is still untested: the rich clipboard (Copy on entries and topics), export — the Markdown zip and the JSON backup — and import. The capture prompt and the port-based panel state are Chromium-only by design. |
...
| **Release prep** | Done: the version is 1.0.0, with a What's Changed page (Settings → About) whose newest entry a test holds to the manifest version; the content script matches only http, https and local files; the licence check ([#183](https://github.com/lexicora/lexicora-extension/issues/183)); the listing copy, permission justifications and screenshots in [`store-listing.md`](store-listing.md); and the privacy policy, in the extension and as [`PRIVACY.md`](../PRIVACY.md), the URL the stores get. **Left:** the Firefox items above; store developer accounts (Chrome Web Store, AMO, Microsoft Partner Center); Chrome's 440×280 small promo tile; `bun run zip` and `bun run zip:firefox` from a clean `main`, installed and clicked through once, with the sources zip for AMO; v1.0.0's date in `src/constants/changelog.ts` on release day, and a `v1.0.0` tag. Submit to Chrome first, then AMO, then Edge. |
